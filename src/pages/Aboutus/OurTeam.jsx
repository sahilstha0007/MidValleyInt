      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-20 py-10">
        ...
        {Object.entries(teamMembers).map(([category, members]) => (
          <div key={category} className="mb-12">
            <h3 className="text-2xl font-semibold text-gray-800 mb-6 capitalize text-center">
              {category}
            </h3>

            {/* Simple divider */}
            <div className="w-24 h-0.5 bg-indigo-500 mx-auto mb-8"></div>

            {members.length === 2 ? (
              // Simple layout for 2 members
              ...
              // Layout for 6 members (4 at top, 2 below)
              <>
              ...
              // Default layout for any number of members
              <div className="flex flex-wrap justify-center gap-6">
                {members.map((member, index) => (
                  <TeamCard key={index} member={member} />
                ))}
...
const TeamCard = ({ member }) => (
  <div className="group bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-all duration-500 flex flex-col items-center justify-between h-[350px] w-full max-w-[250px] sm:max-w-[200px] md:max-w-[220px] lg:max-w-[250px] border border-transparent hover:border-blue-400 transform hover:-translate-y-2">
